...
public class ReservationController {
    ...
    public String payment(@RequestParam Long busId,
                          ...
                          @RequestParam(required = false, defaultValue = "1") int seatsRequested,
                          Model model) {
        Bus bus = busService.get(busId);
        model.addAttribute("bus", bus);
        ...
        model.addAttribute("priceTotal", bus != null ? bus.getPrice() * seatsRequested : 500);
    ...
    public String pay(@RequestParam Long busId,
                      ...
                      @RequestParam String destination,
                      /**
                       * BUG FIX 4: Was String (nullable) — now parsed as int with default 1.
                       * This value is now correctly passed to book() instead of hardcoded 1.
                       */
                      @RequestParam(required = false, defaultValue = "1") int seatsRequested,
        ...
        if (!success) {
            ...
            model.addAttribute("destination", destination);
            model.addAttribute("seatsRequested", seatsRequested);
            model.addAttribute("priceTotal", bus.getPrice() * seatsRequested);
            return "reservations/payment";
        }

        /**
         * BUG FIX 5: Was hardcoded as book(..., 1) — always booked exactly 1 seat
         * regardless of how many seats the user selected. Now uses seatsRequested.
         */
        Reservation r = reservationService.book(user, bus, LocalDate.parse(date), time,
                                                source, destination, seatsRequested);
        if (r == null) {
