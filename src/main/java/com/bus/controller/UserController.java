...
public class UserController {
    ...
    /**
     * BUG FIX 1: "users/login" JSP does not exist.
     * Redirect to the correct existing login page instead.
     */
    @GetMapping("/login")
    ...
    /**
     * BUG FIX 2: Admin.getAdminPassword() previously returned null (broken manual override).
     * Now returns real String value via Lombok after Admin.java fix.
     * Also fixed error redirect to point to a valid page.
     */
    @PostMapping("/login")
    ...
    /**
     * BUG FIX 3: Was redirecting to "/login?registered=true" which mapped
     * to a non-existent "users/login" view. Fixed to redirect to the
     * correct existing login page.
     */
